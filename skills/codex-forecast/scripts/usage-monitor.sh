...
Polls Codex backend usage through the codex-usage helper and writes forecast
...
Environment:
  ...
  CODEX_USAGE_HELPER               Override codex-usage helper path.
EOF
}

state_dir="${CODEX_USAGE_STATE_DIR:-$HOME/.codex/usage}"
helper="${CODEX_USAGE_HELPER:-$HOME/.codex/skills/codex-usage/scripts/show-codex-usage.sh}"
...
write_once() {
  ...
  payload="$(
    jq -n \
      ...
      --arg source "chatgpt backend /backend-api/wham/usage via codex-usage skill" \
  ...
  jq -r '
    ...
    [
      ...
      "Source: ChatGPT backend `/backend-api/wham/usage` via `$codex-usage`.",
      "",
      "## Current",
      "",
